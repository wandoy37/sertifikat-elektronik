...
@section('content')
    <div class="page-inner">
        ...
        <div class="row">
            <div class="col-lg-12">
                <a href="{{ route('orang.create') }}" class="btn btn-primary mb-4">
                                    ...
                                    @foreach ($orangs as $orang)
                                        <tr>
                                            ...
                                            <td class="form-inline">
                                                <a href="{{ route('orang.edit', $orang->id) }}" class="text-primary">
                                                    <i class="fas fa-pen"></i>
                                                </a>
                                                <form id="form-delete-{{ $orang->id }}"
                                                    action="{{ route('orang.destroy', $orang->id) }}" method="post">
                                                    @csrf
                                                    @method('DELETE')
                                                </form>
                                                <button type="button" class="btn btn-link text-danger"
                                                    onclick="btnDelete( {{ $orang->id }} )">
