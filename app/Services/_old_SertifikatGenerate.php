...
class SertifikatGenerate
{
    ...
    public function previewSertifikatBimtek($sertifikat)
    {
        ...
        $pdf->MultiCell(
            266, // lebar cell, sesuaikan dengan lebar halaman
            5.5,  // tinggi per baris
            $sertifikat->judul_kegiatan . ' yang diselenggarakan oleh ' . $sertifikat->penyelenggara_kegiatan . ' mulai tanggal ' . Carbon::parse($sertifikat->tanggal_mulai_kegiatan)->isoFormat('D MMM') . ' s.d. ' . Carbon::parse($sertifikat->tanggal_akhir_kegiatan)->isoFormat('D MMM Y') . ' dengan jumlah ' . $sertifikat->total_jam_kegiatan . ' (' . FacadesTerbilang::make($sertifikat->total_jam_kegiatan) . ') jam pelajaran.',
            0,   // border (0 = tanpa garis)
            'C', // alignment (L = kiri)
            false // fill background
        );
    ...
    public function prosesSingleGenerateBimtek($sertifikat)
    {
        $orang = DB::table('orangs')
            ->where('id', '=', $sertifikat->orang_id)
            ->first();
    ...
    public function prosesAllGenerateBimtek($sertifikats)
    {
        $pdf = new Fpdi();
    ...
    public function prosesSingleTerbitBimtek($sertifikat)
    {
        $orang = DB::table('orangs')
            ->where('id', '=', $sertifikat->orang_id)
            ->first();
